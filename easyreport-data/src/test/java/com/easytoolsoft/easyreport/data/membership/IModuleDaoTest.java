...
public class IModuleDaoTest extends BaseDaoTest<IModuleDao, Module, ModuleExample> {
	...
	protected Module getPo(Integer id) {
		return Module.builder().id(id).parentId(0).name("DaoTest" + id).code(RandomStringUtils.randomAlphanumeric(10))
				.comment("DaoTest" + id).gmtCreated(new Date()).gmtModified(new Date()).hasChild((byte) 0).icon("test")
				.linkType((byte) 0).params("").path("").sequence(10).status((byte) 1).target("").url("").build();
	}
